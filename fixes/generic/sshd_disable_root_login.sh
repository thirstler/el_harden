...
then
    ...
    then
        ...
        echo -e "\nPermitRootLogin ${SSHD_PERMIT_ROOT_LOGIN}" >> $SSHD_CONFIG
    ...
    # Case: PermitRootLogin directive present in $SSHD_CONFIG already
        ...
        sed -i "$FIRST_PERMIT_ROOT_LOGIN s/^[[:space:]]*PermitRootLogin.*$/PermitRootLogin ${SSHD_PERMIT_ROOT_LOGIN}/I" $SSHD_CONFIG
    fi
